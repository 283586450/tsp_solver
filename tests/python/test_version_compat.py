...
class VersionCompatibilityTest(unittest.TestCase):
    def test_python_package_and_native_versions_must_match(self) -> None:
        current_version = re.escape(tsp_solver.__version__)
        with self.assertRaisesRegex(
            VersionMismatchError,
            rf"Python package version {current_version} does not match native library version 9\.9\.9.*same tsp_solver release",
        ):
            _native._ensure_version_match(tsp_solver.__version__, "9.9.9")
    ...
    def test_load_library_rejects_runtime_version_mismatch(self) -> None:
        ...
        try:
            ...
            with mock.patch.object(_native, "package_version", return_value="9.9.9"):
                with self.assertRaisesRegex(
                    VersionMismatchError,
                    rf"Python package version 9\.9\.9 does not match native library version {re.escape(tsp_solver.__version__)}.*same tsp_solver release",
                ):
