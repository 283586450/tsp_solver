...
int main() {
  ...
  assert(tsp_solver_model_add_node(nullptr, &node_id) == TSP_SOLVER_ERROR_INVALID_ARGUMENT);
  assert(tsp_solver_options_set_time_limit_ms(nullptr, 1) == TSP_SOLVER_ERROR_INVALID_ARGUMENT);
  ...
  assert(tsp_solver_solve(nullptr, options, &result) == TSP_SOLVER_ERROR_INVALID_ARGUMENT);
  ...
  assert(tsp_solver_model_set_distance(model, 1, 0, 5) == TSP_SOLVER_ERROR_OUT_OF_RANGE);

  tsp_solver_model_t* incomplete_model = nullptr;
  assert(tsp_solver_model_create(&incomplete_model) == TSP_SOLVER_ERROR_OK);
  assert(tsp_solver_model_add_node(incomplete_model, &node_id) == TSP_SOLVER_ERROR_OK);
  assert(tsp_solver_model_add_node(incomplete_model, &node_id) == TSP_SOLVER_ERROR_OK);
  ...
  assert(tsp_solver_options_set_time_limit_ms(options, 1) == TSP_SOLVER_ERROR_OK);
  assert(tsp_solver_solve(model, options, &result) == TSP_SOLVER_ERROR_OK);

  tsp_solver_node_id_t nodes[1] = {};
  std::size_t written = 0;
  assert(tsp_solver_result_get_tour(result, nodes, 0, &written) ==
         TSP_SOLVER_ERROR_INVALID_ARGUMENT);
  assert(tsp_solver_result_get_tour(result, nodes, 1, &written) == TSP_SOLVER_ERROR_OK);
  assert(written == 1);
